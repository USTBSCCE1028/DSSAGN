import time
from dataloader import OpenbookQADataLoader, DEV_OpenbookQADataLoader
from model import Answer_scoring_module
import torch
import logging
...
if not torch.cuda.is_available:
    ...
    exit(-1)
# =========
KG_NAME = 'CommonsenseQA'
assert KG_NAME in ['CommonsenseQA', 'OpenbookQA ']
# =========
HOPS = 1
...
# =========
if KG_NAME == 'CommonsenseQA':
    QA_PAIRS_PATH = f'../kg_embeddings/CommonsenseQA/{"half_" if KG_HALF else ""}qa_%s_{str(HOPS)}hop.txt'
else:
    QA_PAIRS_PATH = f'../kg_embeddings/OpenbookQA/qa_%s_webqsp.txt'
...
KG_EMBED_PATH = f'../kg_embeddings/{KG_NAME}{"_half" if KG_HALF else "_full"}/' \
...
qa_dataloader = OpenbookQADataLoader(entity_embed_path=E_path, entity_dict_path=entity_dict_path, relation_embed_path=R_path
                                 , relation_dict_path=relation_dict_path, qa_dataset_path=qa_traindataset_path,
                                 batch_size=batch_size)
word_idx = qa_dataloader.dataset.word_idx
qa_dev_dataloader = DEV_OpenbookQADataLoader(word_idx=word_idx, entity_dict_path=entity_dict_path,
                                         relation_dict_path=relation_dict_path,
                                         qa_dataset_path=qa_devdataset_path)
qa_test_dataloader = DEV_OpenbookQADataLoader(word_idx=word_idx, entity_dict_path=entity_dict_path,
                                          relation_dict_path=relation_dict_path,
...
load_from = ''
# ====model init=====
model = Answer_scoring_module(entity_embeddings=entity_embeddings, embedding_dim=embedding_dim, vocab_size=vocab_size,
