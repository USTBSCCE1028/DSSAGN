...
class OpenbookQADataLoader(DataLoader):
    def __init__(self, entity_embed_path, entity_dict_path, relation_embed_path, relation_dict_path, qa_dataset_path,
                 batch_size, split=False, shuffle=True):
        dataset = OpenbookQADataSet(entity_embed_path, entity_dict_path, relation_embed_path, relation_dict_path,
                                qa_dataset_path, split)
        super(OpenbookQADataLoader, self).__init__(dataset=dataset, batch_size=batch_size, shuffle=shuffle,
...
class DEV_OpenbookQADataSet(Dataset):
...
class DEV_OpenbookQADataLoader(DataLoader):
    def __init__(self, word_idx, entity_dict_path, relation_dict_path, qa_dataset_path, split=False, batch_size=128, shuffle=True):
        dataset = DEV_OpenbookQADataSet(word_idx, entity_dict_path, relation_dict_path, qa_dataset_path, split)
        super(DEV_OpenbookQADataLoader, self).__init__(dataset=dataset, batch_size=batch_size, shuffle=shuffle,
                                               collate_fn=self._collate_fn)
